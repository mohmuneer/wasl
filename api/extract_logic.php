...
{
    $f = [
        'customer_name' => null, 'customer_type' => null, 'cr_status' => null,
        'national_id' => null, 'cr_number' => null, 'cr_date' => null,
        'cr_expiry_g' => null, 'capital' => null, 'address' => null,
        'owner_name' => null, 'owner_id' => null, 'phone' => null, 'email' => null,
    ];
    ...
    // رقم هوية المالك
    if (preg_match('/(?:رقم\s*)?(?:الهوية|هوية\s*المالك)\s*[:：]?\s*(\d{10})/u', $clean, $m)) {
        $f['owner_id'] = $m[1];
    }
    ...
    } elseif (mb_strpos($text, 'نشط') !== false) {
        $f['cr_status'] = 'نشط';
    }
    ...
    } elseif (mb_strpos($text, 'مؤسسة') !== false) {
        $f['customer_type'] = 'مؤسسة';
    } elseif (mb_strpos($text, 'شركة') !== false) {
        $f['customer_type'] = 'شركة';
    }

    // ---------- اسم المنشأة ----------
    // يبدأ بكلمة مؤسسة/شركة ويُقطع عند أول كلمة مفتاحية تالية
    ...
    // احتياطي: إن لم نلتقط التواريخ بالكلمات، نرتّب كل التواريخ
    ...
    // ---------- اسم المالك ----------
    foreach ($lines as $i => $line) {
        if (mb_strpos($line, 'اسم المالك') !== false ||
            mb_strpos($line, 'بيانات مالك') !== false ||
            mb_strpos($line, 'مالك المنشأة') !== false) {
            // قد يكون بعد ":" في نفس السطر أو في السطر التالي
            $after = trim(preg_replace('/.*?(?:اسم المالك|بيانات مالك المنشأة|مالك المنشأة)\s*[:：]?/u', '', $line));
            $candidate = $after !== '' ? $after : ($lines[$i + 1] ?? '');
            // يجب أن يكون نصاً عربياً بلا أرقام
            if (mb_strlen($candidate) > 4 && !preg_match('/\d/u', $candidate)) {
