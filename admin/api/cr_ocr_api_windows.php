...
$text = '';
$source = '';
$debug = []; // معلومات تشخيصية تظهر عند الفشل

try {
    if ($ext === 'pdf') {
        $text = pdfToText($tmpPath, $debug);
        $source = 'pdf-text';
        if (mb_strlen(trim($text)) < 30) {
            $text = ocrPdf($tmpPath, $debug);
            $source = 'pdf-ocr';
        }
    } else {
        $text = ocrImage($tmpPath, $debug);
        $source = 'image-ocr';
    }
} catch (Throwable $e) {
...
if (mb_strlen(trim($text)) < 10) {
    respond(422, [
        ...
        'source'  => $source,
        'debug'   => $debug, // يوضّح إن كانت الأداة غير موجودة
    ]);
}
...
function pdfToText(string $path, array &$debug): string
{
    ...
    @exec($cmd, $out, $code);
    if ($code !== 0) {
        $debug[] = "pdftotext فشل (code=$code): " . implode(' ', array_slice($out, 0, 2));
    }
    // عند 2>&1 قد تختلط رسائل الخطأ بالنص؛ نُرجع فقط إن نجح
...
function ocrImage(string $path, array &$debug): string
{
    ...
    @exec($cmd, $out, $code);
    if ($code !== 0) {
        $debug[] = "tesseract فشل (code=$code): " . implode(' ', array_slice($out, 0, 2));
        return '';
    }
    return implode("\n", $out);
}
